---
title: Trying to Find a Flow...
excerpt: >-
  So the whole goal of starting this website was to explore AI: Discovering what
  the state of the art was, and exploring what I could do with it. At least in
  the Large Language Model (LLM) context...
date: 2026-05-09T16:09:00.000Z
categories:
  - design
  - projects
  - finance
...
So the whole goal of starting this website was to explore AI: Discovering what the state of the art was, and exploring what I could do with it. At least in the Large Language Model (LLM) context. And turns out the answer is: You can do a LOT! The capabilities are scary, and at times I wonder if the apocalypse is around the corner and everyone will get put out of a job. The thing that I have noticed is that the tools and models and workflows seem to change daily. Like way too fast for any one person to keep up with (short of it being their full time job). So when I tried coming up with a design/work flow to build my personal fintech app, I found myself throwing out a lot of what I did for this website and mostly starting from scratch. I did pick up some important experience from that build though:

* ***Plan First, Build Later***: My first instinct for building this site was to just YOLO and start building stuff. And to be fair… this is great for brainstorming. But it makes for a hacky product. Many iterations were done and then re-done and then scrapped and re-done again. Not super efficient, and in the end it meant that some of the features and UX design wasn't totally cohesive. Like, I still have to run a script on my laptop to generate the search index and graph constellation map which doesn't make any sense because I can write the blog posts online with keystatic. So going forward, I think new projects need to start with a Product Requirement Document (PRD) that thinks things through from the get go. Lists and prioritizes all the features, and enumerates a product roadmap and milestones.
* ***Context Windows Matter***: I Build this web site in a single claude-code session on Opus 4.6. And I didn't know what a context window was when I started. But as the project got bigger and more complicated, I noticed that the LLM began forgetting stuff. Like important stuff for decisions and pain points in past stages. I guess I would expect an actual human to not be able to handle both the breadth and depth of a large complicated problem… but I never really considered this limitation when dealing with an LLM. I should have though. It's totally the same problem. Which brings me to my next point...
* ***Run your Project like a Business***: And what I mean by that is, define some distinct roles and responsibilities. Figure out milestones and work phases and delegate an LLM agent to oversee the broad task at the highest level. Only. Have it help you define sub-agents that have limited context and scope to perform more specialized roles. Product Manager. UX/UI Designer. Architect. Security Ops. Etc. These all run in separate Sessions and each have their own context windows. Ensure that the overseeing LLM document everything along the way so that the sub-agents are kept up to date on the relevant context.
* ***Recheck the Web Regularly***: The best methods and state of the art changes constantly. Always keep an eye out for youtube videos for new features and better workflow examples. I started this website UX/UI design in Figma because it was used by a big community of designers and had an MCP server that connected with Claude Code. But recently, Anthropic released a new tool called Claude Design which does a lot of the same things. Maybe not quite as good or with as much collaboration support… but you know that is going to change is like a month. Similarly, someone on the podverse was raving about the capabilities of OpenClaw and running sub-agents locally on stacks of Mac Minis. And then a day later Anthropic announced a change in their pricing plan that made this expensive and simultaneously introduced expanded agentic capabilities of their own. It's worth keeping up to date with this stuff.



# Mosko Fintech Project

So where does that leave me now? I'm almost assuredly not at state-of-the-art level of design and development... But I have some good ideas and examples. I want to make a personal fintech app for myself and maybe a few friends and family members. Kind of a bespoke piece of software for a Family Office. I'm going to treat this project like something run in a large corporation. I have some experience with this in a previous life, but it's been more than a decade so there WILL be some hiccups 😂. Roughly the plan is to create a solid workflow\... and through that write a well thought out PRD and Org structure of human(s) and agents.&#x20;

The Primary mechanism for Claude to maintain context and know what it is supposed to do is though two files: CLAUDE.md and WORKFLOW\.md. These files (at the moment) live at the root of my repo and project directory mosko-fintech (If you are wondering what happened to pfin-dash, that is encompassed within the global wrapper of the mosko-fintech organization). The location of the mosko-fintech repository can be found here:\
[https://github.com/richmosko/mosko-fintech](https://github.com/richmosko/mosko-fintech)\


…There are a couple of other files that help, such as the .claude/settings.json (hooks for initial files to read on every new session) and the memory markdown files. Also, all of the sub-agent roles are defined concretely in .claude/agents/\*.md. But this is the workflow that I have so far:



# mosko-fintech CLAUDE.md
