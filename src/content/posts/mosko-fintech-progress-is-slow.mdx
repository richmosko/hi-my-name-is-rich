---
title: 'mosko-fintech: Progress is Slow...'
excerpt: >-
...
I had a plan. A good one in fact. I would do extensive planning up front. I would treat this as a professionally run project: Trackable milestones… Atomic issues tracking… defined breakpoints for PRs and deliverables… Basically, I wanted to run this like I was running a project in a corporate setting. It felt like the right decision, but I think that I have been trying to force-fit a new tool into an old paradigm. I found that I kept feeling like what I was doing was WORK. And in the worst most onerous way. I was not designing or exploring or learning. I felt like I was merely grinding away.

There seems to be too may friction points in my flow.&#x20;

1. I have directed Claude to present me with three different options whenever there was a pivot point for a decision. This seemed reasonable when I was putting together the workflow. I wanted to remain in the decision loop!
2. I also labeled my role as Founder / CTO, and this brought implications for Claude that I really wanted to get my hands dirty for every step along the way.
3. I explicitly told Claude in the Claude.md file that I wanted this project to be run like a mini-corporate project. But this caused it to go way overboard with listed ADRs, Changelist.md tracking, Corporate jargon and process checkpoints and reviews...

Basically, I have set up all of the WORST parts of corporate project planning and handicapped all of the strengths of LLM agents. I have been micro-managing! Ugh. I feel like such a terrible boss / overlord 😂.

I am pretty far through the mosko-fintech meta project right now… but I'm leary to redefine my workflow wholesale through this process. Ideally, I'd redefine a few things such that a gave Claude much more agency to decide details on design as well as implementation. After all, It's kind of trivial to tell an LLM agent at the end of a checkpoint that you don't like X/Y/Z about the design and have it perform a redesign or refinement loop. This would be a better granularity for human review and decision making. So what I have done instead is go down two different paths:

* ***Refine the mosko-fintech repo***: \
  I've cleaned up as much as I can without totally confusing Claude in it's current development flow. I've set the decision permissions to "auto-mode" so that I don't have to approve every little thing. I've defined milestones that are atomic that map to major decision points and match Project tracking concepts like Linear Issues. This helps automate things. I basically always go with the default recommendation now, and only really review the final results at milestone boundaries. Not perfect… but a little less hands on.
* ***Create a New project-template repo***: \
  This is a meta project template for how I would do things if i were to start from scratch. It's clean. It's more organized. It gives Claude more agency and only asks for input and approval at milestone boundaries. Pull Requests are approved by human review, but this is also an automated skill. Milestones are tracked professionally in Linear, so a human can see the workflow progress at a glance on a phone app. Also, this (in theory) allows massive parallelization of agents on sub-tasks since the issues / milestones are atomic in scope. And agents can comment on and close issues (and assign each other issues!) without human intervention.\
  Most importantly though, whenever I have seen a pain point in the mosko-finance process, I have added the finding to the directives in the project-template repo. I feel like it's a pretty solid reference and starting point. It also seems to be at the state of the art, with support for team-agents and /goals workflow support. Documentation is native in HTML, so documents can support more dynamic content. Common skills with GitHub and other MCPs are pre-defined.

# Mosko Fintech Development State

PRD is defined. The baseline task for V1.x is to replicate my existing financial tracking (in Google Sheets / Docs) as an online dashboard with a supabase - postgres backend. Data acquisition will be though Plaid… which will cost some money at first. There is hope that a new open standard in US finance will be mandated in a year or 3… but for now, Plaid simply works for obtaining financial records. This is in HTML.\


ARCH document is also defined. This is in HTML as well, which is beautiful when locally rendered. But it does not show at all in GitHub except as raw HTML text. Will need a better viewing methodology from GitHub.&#x20;



Process workflow has been dry-run and seems to have the MCP connections and agent calls and delegation working well. Actual backend implementation and deployment is under way.



# \[project-teplate] Repo

The README.md gives a fairly good idea of how the refined workflow works. The repo reference is here:\
[https://github.com/richmosko/project\_template](https://github.com/richmosko/project_template)\
\
...
> 7. Spawn the Research team: *"Create an agent team for the Research phase."*
